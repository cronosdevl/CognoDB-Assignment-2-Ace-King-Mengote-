...
The shortest route through the role ladder, with the skill gap at each step —
each one carrying a *head start* (an adjacent skill they already hold) and a
named colleague who could teach it, preferring someone they have shipped with.

![Career pathfinder](docs/screenshots/pathfinder.png)

### Overview

![Overview](docs/screenshots/overview.png)

### Person detail — the collaboration network

There is no `COLLEAGUE` relationship in the graph; this is
`Person → Project ← Person` plus mentorship, laid out with d3-force and drawn as
keyboard-navigable SVG.

![Person detail](docs/screenshots/person.png)

### Key-person risk

![Key-person risk](docs/screenshots/risk.png)

### Project detail — coverage, gaps and hidden experts

![Project detail](docs/screenshots/project.png)

### Dark mode

Light and dark are the same design tokens re-pointed, applied before first paint.

![Overview in dark mode](docs/screenshots/overview-dark.png)

<details>
<summary>Remaining screens</summary>

| | |
| --- | --- |
| ![People](docs/screenshots/people.png) | ![Projects](docs/screenshots/projects.png) |
| ![Skills](docs/screenshots/skills.png) | ![Skill detail](docs/screenshots/skill.png) |
| ![Connections](docs/screenshots/connections.png) | ![Pathfinder dark](docs/screenshots/pathfinder-dark.png) |

</details>

All captures are generated by `npm run screenshots --workspace client`, which
loads every screen against the live database, fails the build on any console
error, and writes the images above. It is a smoke test first: a React runtime
error does not break `tsc` or `vite build`, so without rendering the pages a
broken screen ships silently.
...
| `npm run screenshots --workspace client` | Render every screen, fail on console errors, write `docs/screenshots/`. |
